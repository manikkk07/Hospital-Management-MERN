...
const Appointments = ({ appointments }) => {
  ...
  // Filter out invalid appointments BEFORE mapping
  const validAppointments = appointments.filter(item => item && item.user);
    ...
    <table className="w-full text-left text-sm text-gray-500">
        ...
        <tbody>
            ...
            <tr key={item._id}>
                ...
                <td className="px-6 py-4">{item.user?.gender || 'Not specified'}</td>
                <td className="px-6 py-4">
                    {item.isPaid ? (
                        <div className="flex items-center">
                            <div className="h-2.5 w-2.5 rounded-full bg-green-500 mr-2"></div>
                ...
                <td className="px-6 py-4">₹{item.ticketPrice || 0}</td>
                <td className="px-6 py-4">{formateDate(item.createdAt)}</td>
            </tr>
            ))}
        </tbody>
